...
本系统是一套基于计算机视觉技术的智能化小鼠管理系统，核心特色包括：
- **二维码笼子管理**：通过二维码实现笼子的快速注册和激活
- **计算机视觉识别**：基于深度学习的小鼠自动识别和行为分析
- **全流程自动化**：从数据采集到报告生成的完整自动化流程
...
public class CageController {
    ...
    public AjaxResult registerCage(@RequestBody CageRegisterDto dto) {
        // 笼子注册，生成二维码
        // 返回二维码图片和笼子ID
    }
    
    @PostMapping("/activate")
    public AjaxResult activateCage(@RequestBody CageActivateDto dto) {
        // 通过扫描二维码激活笼子
        // 绑定小鼠信息，启动监控
    }
    ...
    @GetMapping("/list")
    public TableDataInfo getCageList(CageQueryDto query) {
        // 获取笼子列表，支持筛选
    }
}
```
